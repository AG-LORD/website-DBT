...
export default function Achievements() {
  ...
  const loadAchievements = async () => {
    // Mock data simulating fetched achievements
    const mockAchievements = [
      { achievement_type: "quiz_master", points_earned: 50, earned_date: "2023-01-01" },
      { achievement_type: "perfect_scholar", points_earned: 100, earned_date: "2023-02-01" },
      { achievement_type: "learning_streak", points_earned: 30, earned_date: "2023-02-10" },
    ];

    // Sort by earned_date descending
    mockAchievements.sort((a, b) => new Date(b.earned_date) - new Date(a.earned_date));

    setAchievements(mockAchievements);

    const totalPoints = mockAchievements.reduce((sum, ach) => sum + (ach.points_earned || 0), 0);
